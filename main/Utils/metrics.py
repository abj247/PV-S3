...
def batch_intersection_union(output, target, num_class):
    ...
    return area_inter.cpu().numpy(), area_union.cpu().numpy(), area_pred.cpu(), area_lab.cpu()
...
def eval_metrics(output, target, num_classes, ignore_index):
    ...
    inter, union, pred, lab = batch_intersection_union(output.data, target, num_classes)
    return [np.round(correct, 5), np.round(labeled, 5), np.round(inter, 5), np.round(union, 5), np.round(pred, 5), np.round(lab, 5)]
