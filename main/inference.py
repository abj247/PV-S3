...
from DataLoader.pvs3 import EL
import json
...
el_class_name = ['Crack', 'Interconnect', 'Contact', 'Corrosion']


def get_el_pallete(num_classes):
    n = num_classes
    pallete = [0] * (n * 3)
    for j in range(0, n):
        lab = j
...
def running_inference(loader, model, folder_name, save_img=False):
    ...
    statistic_conf_bar = torch.zeros([4, ], dtype=torch.float).cuda(non_blocking=True)
    num_record = torch.zeros([4, ], dtype=torch.float).cuda(non_blocking=True)
    ...
    plt.xlim(-1, 5)
    plt.xticks(ticks=range(0, 4), labels=el_class_name, rotation='vertical')
    plt.bar(range(0, 4), (statistic_conf_bar/num_record).detach().cpu().numpy(), width=0.5)
...
def main():
    ...
    # DATA
    loader = EL(config['val_loader'])
    config['model']['resnet'] = args.backbone
    config['model']['data_h_w'] = [0, 0]

    # MODEL
    model = model_deep(num_classes=5, config=config['model'],
