...
class Trainer(BaseTrainer):
    ...
    def _warm_up(self, epoch, id):
        self.model.train()
        assert id == 1 
        #or id == 2 or id == 3, "Expect ID in 1, 2 or 3"
        ...
        for batch_idx in tbar:
            ...
            with autocast():
                ...
                total_loss, cur_losses, outputs = self.model(x_l=input_l, target_l=target_l,
                                                             ...
                                                             id=id, semi_p_th=self.args.semi_p_th, 
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(tbar):
                ...
                with autocast(): 
                    ...
                    correct, labeled, inter, union, pred, lab = eval_metrics(output, target, self.num_classes, self.ignore_index)
                    total_inter, total_union = total_inter + inter, total_union + union
                    total_correct, total_label = total_correct + correct, total_label + labeled
                    total_pred, total_lab = total_pred + pred, total_lab + lab

                    # Assuming 'output' is your model's softmax/logits output and 'target' is your ground truth
                    # Convert 'output' to predicted classes
                    _, predicted = torch.max(output, 1)
                    predicted = predicted.cpu().numpy()
                    target = target.cpu().numpy()

                    # Flatten the arrays to list and extend the accumulative lists
                    ...
                    prec = 1.0 * total_inter / (np.spacing(1) + total_pred)
                    recall = 1.0 * total_inter / (np.spacing(1) + total_lab)
                    f1 = 2.0 * (prec * recall) / (prec + recall) 
                    mIoU = IoU.mean().item()
                    mPrec = prec.mean().item()
                    mRecall = recall.mean().item()
                    mF1 = 2.0 * (mPrec * mRecall) / ((mPrec+1) + mRecall)

                    # Calculate confusion matrix in a more efficient manner
                    # if batch_idx % self.log_step == 0 or batch_idx == len(tbar) - 1:
                    #     conf_matrix = confusion_matrix(all_targets, all_preds, labels=range(self.num_classes))
                    # else:
                    #     conf_matrix = None

                    seg_metrics = {"Pixel_Accuracy": np.round(pixAcc, 4), "Mean_IoU": np.round(mIoU, 4),
                                   "Mean_Precision": np.round(mPrec, 4), "Mean_Recall": np.round(mRecall, 4), "Mean_F1": np.round(mF1, 4),
                                   "Class_IoU": dict(zip(range(self.num_classes), np.round(IoU, 4))),
                                   "Class_Prec": dict(zip(range(self.num_classes), np.round(prec, 4))),
                                   "Class_Recall": dict(zip(range(self.num_classes), np.round(recall, 4))),
                                   "Class_F1": dict(zip(range(self.num_classes), np.round(f1, 4)))}
            ...
            log = {
                ...
                #'confusion_matrix': conf_matrix if conf_matrix is not None else "Not Calculated in this iteration"
                **seg_metrics
            }
        return log
    ...
    #             # Printing interim results for debugging
    #             pixAcc = 1.0 * total_correct / (np.spacing(1) + total_label)
    #             IoU = 1.0 * total_inter / (np.spacing(1) + total_union)
    #             tbar.set_description('EVAL ID ({}) ({}) | Loss: {:.4f}, PixelAcc: {:.4f}, Mean IoU: {:.4f} |'.format(
    #                 "Teachers",
    ...
    #         # Integrating confusion matrix into seg_metrics
    #         seg_metrics.update({
    ...
    def _reset_metrics(self):
        ...
        self.total_pred_l, self.total_lab_l = 0, 0
        self.total_pred_ul, self.total_lab_ul = 0, 0
        self.total_correct_ul, self.total_label_ul = 0, 0
        self.mIoU_l, self.mIoU_ul = 0, 0
        self.pixel_acc_l, self.pixel_acc_ul = 0, 0
        self.prec_l, self.prec_ul = 0, 0
        self.recall_l, self.recall_ul = 0, 0
        self.f1_l, self.f1_ul = 0, 0
        self.class_iou_l, self.class_iou_ul = {}, {}
        self.class_prec_l, self.class_prec_ul = {}, {}
        self.class_recall_l, self.class_recall_ul = {}, {}
        self.class_f1_l, self.class_f1_ul = {}, {}
    ...
    def _compute_metrics(self, outputs, target_l, target_ul, sup=False):
        ...
        self.pixel_acc_l,self.mIoU_l, self.prec_l, self.recall_l, self.f1_l, self.class_iou_l, self.class_prec_l, self.class_recall_l, self.class_f1_l = seg_metrics_l.values()
        ...
        if self.mode == 'semi':
            ...
            self.pixel_acc_ul, self.mIoU_ul, self.prec_l, self.recall_l, self.f1_l, self.class_iou_ul, self.class_prec_ul, self.class_recall_ul, self.class_f1_ul = seg_metrics_ul.values()

    def _update_seg_metrics(self, correct, labeled, inter, union, pred, lab, supervised=True):
        if supervised:
            self.total_correct_l += correct
            self.total_label_l += labeled
            self.total_inter_l += inter
            self.total_union_l += union
            self.total_pred_l += pred
            self.total_lab_l += lab
            ...
            self.total_pred_ul += pred
            self.total_lab_ul += lab
    ...
    def _get_seg_metrics(self, supervised=True):
        if supervised:
            ...
            prec = 1.0 * self.total_inter_l / (np.spacing(1) + self.total_lab_l)
            recall = 1.0 * self.total_inter_l / (np.spacing(1) + self.total_pred_l)
            f1 = 2.0 * (prec * recall) / prec + recall
        else:
            ...
            prec = 1.0 * self.total_inter_ul / (np.spacing(1) + self.total_lab_ul)
            recall = 1.0 * self.total_inter_ul / (np.spacing(1) + self.total_pred_ul)
            f1 = 2.0 * (prec * recall) / (prec + recall)
        mIoU = IoU.mean()
        mPrec = prec.mean()
        mRecall = recall.mean()
        ...
        return {
            ...
            "Mean_Prec": np.round(mPrec, 3),
            "Mean_Recall": np.round(mRecall, 3),
            "mF1": np.round(mF1, 3),
            "Class_IoU": dict(zip(range(self.num_classes), np.round(IoU, 3))),
            "Class_Prec": dict(zip(range(self.num_classes), np.round(prec, 3))),
            "Class_Recall": dict(zip(range(self.num_classes), np.round(recall, 3))),
            "Class_F1": dict(zip(range(self.num_classes), np.round(f1, 3)))
        }
